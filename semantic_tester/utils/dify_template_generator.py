...
class DifyTemplateGenerator:
    ...
    def generate_basic_template(self, supplier: str = "dify") -> Optional[str]:
        """
        ...
        """
        # 根据供应商确定响应列名
        response_col = self.SUPPLIER_RESPONSE_COLUMNS.get(
            supplier.lower(), f"{supplier}响应"
        )
    ...
    def generate_multi_supplier_template(self, suppliers: List[str]) -> Optional[str]:
        """
        生成多供应商模板
    ...
    def _create_multi_supplier_sample_data(
        self, columns: List[str], suppliers: List[str]
    ) -> Dict[str, Any]:
        """创建多供应商示例数据"""
        sample_data = {
            "时间戳": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def _get_sample_response(self, supplier: str) -> str:
        ...
        return responses.get(
            supplier.lower(), "这是一个AI回答示例。请根据您的具体需求修改此内容。"
        )
...
def create_dify_template_interactive():  # noqa: C901
    """交互式创建Dify模板"""
    ...
    if choice == "1":
        ...
        try:
            supplier_choice = int(
                input(
                    f"{Fore.YELLOW}请选择 (1-{len(generator.SUPPLIER_RESPONSE_COLUMNS)}): {Style.RESET_ALL}"
                )
            )
        ...
        supplier_input = input(
            f"{Fore.YELLOW}请输入序号 (例如: 1,3,5): {Style.RESET_ALL}"
        ).strip()
        try:
            indices = [int(x.strip()) - 1 for x in supplier_input.split(",")]
            suppliers = list(generator.SUPPLIER_RESPONSE_COLUMNS.keys())
            selected_suppliers = [
                suppliers[i] for i in indices if 0 <= i < len(suppliers)
            ]
