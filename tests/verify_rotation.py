...
def test_rotation_configuration():
    ...
    if iflow is None:
        logger.error(
            f"❌ iFlow provider is None. Available providers: {list(manager.providers.keys())}"
        )
...
def test_rotation_logic():
    ...
    with patch.object(GeminiProvider, "validate_api_key", return_value=True):
        ...
        if initial_key_index != new_key_index:
            logger.info(
                f"✅ Gemini rotated key: {initial_key_index} -> {new_key_index}"
            )
        else:
    ...
    with patch.object(GeminiProvider, "validate_api_key", return_value=True):
        ...
        if initial_key_index == new_key_index:
            logger.info(
                f"✅ Gemini did not rotate key: {initial_key_index} -> {new_key_index}"
            )
        else:
            logger.error(
                f"❌ Gemini rotated key unexpectedly: {initial_key_index} -> {new_key_index}"
            )
    ...
    with patch.object(OpenAIProvider, "validate_api_key", return_value=True):
        ...
        if initial_key_index == new_key_index:
            logger.info(
                f"✅ OpenAI did not rotate key: {initial_key_index} -> {new_key_index}"
            )
        else:
            logger.error(
                f"❌ OpenAI rotated key unexpectedly: {initial_key_index} -> {new_key_index}"
            )
