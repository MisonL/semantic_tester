...
1. 在当前目录下创建 `处理后/` 文件夹（或自定义名称）。
2. 将知识库文档放入该目录中，支持：`.txt` / `.md` / `.pdf` 等常见格式。
3. 程序运行时会自动扫描该目录中的所有文档。
...
3. 程序会自动加载您指定的文档目录进行语义匹配，并在指定位置生成结果 Excel 文件。
...
#### 💡 核心配置：多渠道高并发架构 (v4.0.0+)

v4.0.0 引入了全新的多渠道配置系统，支持同时驱动多个 API 密钥实现极速处理。
...
# 渠道 2 (Gemini)
AI_CHANNEL_2_NAME=Gemini-Flash
AI_CHANNEL_2_API_KEY=your-gemini-key
AI_CHANNEL_2_CONCURRENCY=2
```

详细的多渠道配置策略请参考 [README.md](../README.md)。
...
1. 在项目根目录下创建一个名为 `处理后/` 的文件夹（项目默认使用此目录）。
