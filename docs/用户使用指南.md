...
## 0. 🚀 打包版本快速上手（Windows / macOS）

如果您是直接下载的打包版本（如 `semantic_tester_windows_v*.zip` 或 `semantic_tester_macos_v*.tar.gz`），推荐先阅读本章节。

### 0.1 解压与目录结构

- 将压缩包解压到一个**不包含中文和空格**的路径，如：
  - Windows：`D:\Tools\SemanticTester`
  - macOS：`~/Tools/semantic_tester`
- 解压后的目录中通常包含：
  - Windows：`semantic_tester.exe` 可执行文件
  - macOS：`semantic_tester` 可执行二进制文件
  - `.env.config.example` 配置示例文件
  - `使用说明.md`（即本指南）
  - （可选）其他说明或示例文件

### 0.2 配置环境（两端一致）

1. 在解压目录中找到 `.env.config.example`。
2. 复制一份并重命名为 `.env.config`（注意文件名前有一个点）：
   - Windows 用户请确认没有变成 `.env.config.txt`。
3. 使用文本编辑器（记事本 / VSCode 等）打开 `.env.config`，按注释填入：
   - API Key（如 iFlow、OpenAI 等）
   - 其他必要配置（如模型名称、代理等）。

### 0.3 准备知识库

1. 在当前目录下创建 `kb-docs` 文件夹（如果不存在）。
2. 将知识库文档放入该目录中，支持：`.txt` / `.md` / `.pdf` 等常见格式。
3. 程序运行时会自动扫描 `kb-docs` 中的所有文档。

### 0.4 启动程序

#### Windows（打包后的 `semantic_tester.exe`）

1. 在资源管理器中进入解压后的目录。
2. 双击 `semantic_tester.exe` 启动程序。
3. 若出现 Windows Defender 提示：
   - 点击“更多信息” → “仍要运行”。

#### macOS（打包后的 `semantic_tester` 二进制文件）

1. 在 Finder 中进入解压后的目录。
2. 第一次运行建议：
   - 按住 `Control` 键并单击 `semantic_tester` → 选择“打开”。
   - 在弹出的对话框中再次点击“打开”，以绕过“未验证开发者”的限制。

> 提示：如果双击无法打开，可在终端中进入该目录，执行：
>
> ```bash
> ./semantic_tester
> ```

### 0.5 基本使用流程（打包版与源码版一致）

1. 根据命令行提示选择：
   - 批量测试模式 / 单次测试模式等。
2. 指定输入 Excel 或交互式输入问题。
3. 程序会自动加载 `kb-docs` 中的文档进行语义匹配，并在当前目录生成结果 Excel 文件。
...
> 提示：首次运行程序或解压打包版本时，如果当前目录下没有 `.env.config`，程序会自动：
>
> - 优先复制同目录下的 `.env.config.example` 生成 `.env.config`
> - 若没有找到模板，则根据内置默认配置自动生成一个 `.env.config`
>
> 您只需要找到并编辑这个自动生成的 `.env.config` 文件即可开始使用。
...
AI_CHANNEL_1_API_KEY=sk-your-key
AI_CHANNEL_1_CONCURRENCY=10

# 兼容模式配置
IFLOW_API_KEY=sk-your-key
IFLOW_MODEL=qwen3-max
```

**如何获取 iFlow API Key：**

1. 访问 [iFlow 官网](https://platform.iflow.cn/profile?tab=apiKey)
2. 注册并登录账户
3. 进入"API Keys"页面
